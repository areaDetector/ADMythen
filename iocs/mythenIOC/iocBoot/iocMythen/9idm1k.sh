...
# Manually set IOC_STARTUP_DIR if slsDetectorApp.sh will reside somewhere other than iocslsDetectorApp
#!IOC_STARTUP_DIR=/home/username/epics/ioc/synApps/slsDetectorApp/iocBoot/iocslsDetectorApp
IOC_NAME=9idm1k
PROC_NAME=mythenV3App
...
checkpid() {
    MY_UID=`${ID} -u`
    # The '\$' is needed in the pgrep pattern to select slsDetectorApp, but not slsDetectorApp.sh
    IOC_PID=`${PGREP} ${PROC_NAME}\$ -u ${MY_UID}`
...
start() {
    if checkpid; then
        ...
        cd ${IOC_STARTUP_DIR}
	./in-screen.sh
	# Run slsDetectorApp outside of a screen session, which is helpful for debugging
	#!./run
    fi
}
